...
func TestProjectionResidualManifestAgainstPinnedSuite(t *testing.T) {
	suite, err := conformance.LoadSuite(filepath.Join("..", "..", "testdata", "reference", conformance.ReferenceName))
	if err != nil {
		t.Fatal(err)
	}
	report := conformance.RunWithOptions(suite, realSuiteCompiler{}, conformance.ProjectionResidualManifest, conformance.Options{
	...
	if len(report.Failures) != 0 {
		t.Fatalf("projection residual conformance failures: %+v", report.Failures)
	}
	if report.EnabledCases != 20 || report.Passes != report.EnabledCases {
		t.Fatalf("enabled=%d passes=%d, want 20 passing cases", report.EnabledCases, report.Passes)
	}
